...
def test_add_attributes_to_data_array():
    ...
    assert_data_array_eq(attributed, make_raster(np.full((8, 8), 42), attrs=dict(some='attribute', added='attribute')))
...
def test_add_new_attribute_based_on_xarray():
    ...
    assert_data_array_eq(attributed, make_raster(np.full((8, 8), 42), attrs=dict(the_answer_is=42)))
